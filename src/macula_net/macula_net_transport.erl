...
%%%   <li>Caller registers an inbound handler via the
%%%       <code>set_handler/1</code> callback; the transport calls
%%%       this for every CBOR envelope it receives.</li>
%%%   <li>Caller establishes outbound links via the impl's <code>connect</code>
%%%       function (impl-specific).</li>
%%%   <li>The <code>send/2</code> callback delivers an encoded
%%%       envelope to a known station.</li>
%%% </ol>
%%% @end
%%%-------------------------------------------------------------------
