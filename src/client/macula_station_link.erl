...
handle_info({quic, Bin, Stream, _Flags}, #state{stream_bufs = Bufs} = S)
    ...
    {noreply, dedicated_items(macula_frame:parse_stream(<<Buf/binary, Bin/binary>>), Stream, S)};
...
handle_info({quic, Bin, Stream, _Flags},
        ...
        when is_binary(Bin), is_map_key(Stream, Bufs) ->
    Buf = maps:get(Stream, Bufs),
    {noreply, content_items(macula_frame:parse_stream(<<Buf/binary, Bin/binary>>), Stream, S)};
...
%% What `macula_frame:parse_stream/1' gave for a content stream, handled as
...
%% What `macula_frame:parse_stream/1' gave for a dedicated stream: its frames
