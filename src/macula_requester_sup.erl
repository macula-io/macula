%%%-------------------------------------------------------------------
%%% @doc Factory supervisor for `macula_requester' children.
%%%
%%% Unlike `macula_responder_sup' (owned internally by the SDK, one per
...
init([]) ->
    ChildSpec = #{
        id => macula_requester,
        start => {macula_requester, start_link, []},
        restart => temporary,
        shutdown => 5_000,
        type => worker,
        modules => [macula_requester]
    },
