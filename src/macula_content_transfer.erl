...
%% `stream_count' (see `start_put/3') plus the station this dial must
%% prove, `expected_node_id' (see `macula:put_content_station/5').
%% `pin_tls_cert' and `verify' are read by nothing, and `macula' refuses
%% them; see `macula:trust_options_checked/1'.
-spec start_put_station(macula:pool(), macula_client:seed(), binary(),
                        pos_integer(), map()) -> {ok, pid()}.
