...
do_call(not_found, BinaryProcedure, Args, Opts, _From, State) ->
    %% Authorization disabled until UCAN infrastructure is implemented.
    %% The previous check blocked ALL cross-node RPC calls because no node
    %% owns another node's namespace and no UCAN tokens exist yet.
    %% TODO: Re-enable when UCAN token exchange is implemented.
    Registry = State#state.service_registry,
    DiscoveryResult = macula_service_registry:discover_service(Registry, BinaryProcedure),
    do_call_discovery(DiscoveryResult, BinaryProcedure, Args, Opts, _From, State).
...
%% NOTE: get_caller_did/2 removed — RPC authorization disabled until UCAN is implemented.
%% Restore when re-enabling auth in do_call/6.
