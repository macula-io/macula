...
apply_gossip_updates(#{member_list := List, gossip := Gossip} = State, Updates) ->
    ...
    List2 = lists:foldl(fun apply_gossip_update/2, List, Updates),
...
%% @doc Apply a gossip update to the member list.
apply_gossip_update({NodeId, Status, Incarnation}, AccList) ->
    case macula_membership_list:get_member(AccList, NodeId) of
        {ok, ExistingMember} ->
            update_existing_member(AccList, ExistingMember, Status, Incarnation);
        not_found ->
            add_new_member(AccList, NodeId, Status, Incarnation)
    end.

%% @doc Update existing member with new status.
update_existing_member(AccList, Member, alive, Incarnation) ->
    Updated = macula_membership_member:mark_alive(Member, Incarnation),
    macula_membership_list:update_member(AccList, Updated);
update_existing_member(AccList, Member, suspect, _Incarnation) ->
    Updated = macula_membership_member:mark_suspect(Member),
    macula_membership_list:update_member(AccList, Updated);
update_existing_member(AccList, Member, dead, _Incarnation) ->
    Updated = macula_membership_member:mark_dead(Member),
    macula_membership_list:update_member(AccList, Updated).

%% @doc Add new member to list (unless already dead).
add_new_member(AccList, _NodeId, dead, _Incarnation) ->
    %% Don't add already-dead members
    AccList;
add_new_member(AccList, NodeId, Status, Incarnation) ->
    %% Create member with placeholder address (should come from discovery)
    NewMember = macula_membership_member:new(NodeId, {{0,0,0,0}, 0}),
    Updated = apply_initial_status(NewMember, Status, Incarnation),
    macula_membership_list:add_member(AccList, Updated).

%% @doc Apply initial status to new member.
apply_initial_status(Member, alive, Incarnation) ->
    macula_membership_member:mark_alive(Member, Incarnation);
apply_initial_status(Member, suspect, _Incarnation) ->
    macula_membership_member:mark_suspect(Member).
