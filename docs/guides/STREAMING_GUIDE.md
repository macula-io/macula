...
right below. `call_stream/5`/`advertise_stream/5` (the raw primitives) are
covered further down, for anyone building something the wrapper doesn't fit.

---

## Supervised wrappers: `macula_streamer` / `macula_stream_sink`

The bare handler fun (covered in [Provider side](#provider-side), below) runs
in a transient process per inbound STREAM_OPEN, and a consumer has to
hand-write its own `recv/2` loop around `call_stream/5`'s raw stream pid
(covered in [Consumer side](#consumer-side)). `macula_streamer` and
...
The raw primitive [`macula_stream_sink` wraps](#supervised-wrappers-macula_streamer--macula_stream_sink),
...
The raw primitive [`macula_streamer` wraps](#supervised-wrappers-macula_streamer--macula_stream_sink),
above. Reach for it directly only if you're building something the wrapper
doesn't fit.
