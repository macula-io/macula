...
### v0.8.6 (Documentation & Polish)
- Bug fixes from v0.8.5 feedback
- **API Module Naming Consistency** - Docs show `macula:advertise/2` but code requires `macula_client:advertise/2`
  - Either create `macula.erl` facade module with clean API
  - Or update all documentation to use `macula_client` module
  - Decision: Prefer `macula` module as single entry point (Platform Vision alignment)
- **C4 Diagram SVG Conversion** - Convert all ASCII C4 diagrams to professional SVG images
  - Level 1: System Context Diagram
  - Level 2: Container Diagram
  - Level 3: Component Diagrams (Mesh Services, Protocol Layer)
  - Level 4: Code Diagram (Connection Management)
  - File: `architecture/macula_http3_mesh_c4_diagrams.md`
- **Convert io:format to Structured Logging** - Replace all io:format calls with proper logger calls
  - Use `logger:info/2`, `logger:debug/2`, `logger:warning/2`, `logger:error/2`
  - Add structured metadata where appropriate
  - Enables log filtering, levels, and production observability
