...
Public API that applications use:
- `advertise/3,4` - Advertise a service
- `call/2,3` - Call a service
...
Application
   ↓
macula_client:advertise(Client, Procedure, Handler, Opts)
   ↓
...
re_advertise_loop(Client, Procedure, Handler, Opts) ->
    ...
    {ok, _Ref} = macula_client:advertise(Client, Procedure, Handler, Opts),
...
Service discovery happens **automatically** when calling a service via `macula_client:call/2,3`.
...
{ok, _} = macula_client:advertise(Client, <<"calc.add">>, HandlerFn),

%% Node A calls the same service - executed locally (no network)
{ok, Result} = macula_client:call(Client, <<"calc.add">>, #{a => 10, b => 5}).
%% Result = #{result => 15}
```
...
```erlang
case macula_client:call(Client, Procedure, Args, #{timeout => 10000}) of
    {ok, Result} ->
        %% Success
...
call_with_retry(Client, Procedure, Args, MaxRetries, Attempt) ->
    case macula_client:call(Client, Procedure, Args, #{timeout => 5000}) of
        {ok, Result} ->
            {ok, Result};
...
%% Make calls
{ok, #{result := 15}} = macula_client:call(
...
```erlang
%% Provider Node 1
{ok, Client1} = macula_client:connect(<<"https://node1:9443">>, #{}),
{ok, _} = macula_client:advertise(Client1, <<"weather.get">>, Handler1),

%% Provider Node 2
{ok, Client2} = macula_client:connect(<<"https://node2:9443">>, #{}),
{ok, _} = macula_client:advertise(Client2, <<"weather.get">>, Handler2),

%% Consumer Node
{ok, Client3} = macula_client:connect(<<"https://node3:9443">>, #{}),

%% Call service - DHT returns both providers
%% One is selected automatically
{ok, Weather} = macula_client:call(
    Client3,
    <<"weather.get">>,
    #{city => <<"Brussels">>}
).
...
defmodule MyApp.MaculaRPC do
  ...
  defp advertise_services(client) do
    ...
    :macula_client.advertise(client, "myapp.users", user_handler, %{
    ...
    :macula_client.advertise(client, "myapp.posts", post_handler, %{
      metadata: %{version: "1.0.0"}
    })
  end
  ...
  def call(procedure, args, opts \\ %{}) do
    ...
    :macula_client.call(client, procedure, args, opts)
  end
...
| **Registration** | `session.register(Procedure, Handler)` | `macula_client:advertise(Client, Procedure, Handler)` |
| **RPC Call** | `session.call(Procedure, Args)` | `macula_client:call(Client, Procedure, Args)` |
| **Unregister** | `session.unregister(Registration)` | `macula_client:unadvertise(Client, Procedure)` |
...
{ok, Ref} = macula_client:advertise(Client, <<"myapp.user.get">>, Handler).
```
...
%% After (Macula)
{ok, Result} = macula_client:call(Client, <<"myapp.user.get">>, #{user_id => <<"123">>}).
```
...
%% After (Macula)
case macula_client:call(Client, Procedure, Args) of
    {ok, Result} -> handle_result(Result);
...
- [ ] Replace `bondy:register/3` with `macula_client:advertise/3,4`
- [ ] Replace `bondy:call/3` with `macula_client:call/2,3`
- [ ] Replace `bondy:unregister/2` with `macula_client:unadvertise/2`
...
- [Protocol Specification](macula_http3_mesh_protocol_spec.md) - Wire protocol details
- [Quick Start Guide](macula_http3_mesh_quick_start.md) - Getting started tutorial
- [DHT Service Advertisement Status](../DHT_SERVICE_ADVERTISEMENT_STATUS.md) - Implementation status

---

**Last Updated**: 2025-01-10
**Status**: ✅ Complete (Phase 3 - DHT Integration)
