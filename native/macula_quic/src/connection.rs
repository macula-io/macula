...
fn close_reason_term<'a>(env: Env<'a>, reason: Option<quinn::ConnectionError>) -> Term<'a> {
    ...
    match reason {
        ...
        Some(ConnectionError::TransportError(error)) => (
            ...
            u64::from(error.code),
            owned_binary(env, error.reason.as_bytes()),
        )
            .encode(env),
...
/// `bytes` as an Erlang binary; an empty binary if the VM cannot allocate one.
fn owned_binary<'a>(env: Env<'a>, bytes: &[u8]) -> Term<'a> {
    match rustler::OwnedBinary::new(bytes.len()) {
        Some(mut binary) => {
            binary.as_mut_slice().copy_from_slice(bytes);
            binary.release(env).encode(env)
        }
        None => rustler::Binary::new(env, &[]).encode(env),
    }
}
