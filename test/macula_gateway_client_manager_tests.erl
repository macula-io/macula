%%%-------------------------------------------------------------------
%%% @doc
%%% EUnit tests for macula_gateway_client_manager module.
...
cleanup(Pid) ->
    case erlang:is_process_alive(Pid) of
        true -> macula_gateway_client_manager:stop(Pid);
        false -> ok
    end.
...
module_exports_test() ->
    Exports = macula_gateway_client_manager:module_info(exports),
...
stop_test() ->
    {ok, Pid} = macula_gateway_client_manager:start_link(#{}),
    ok = macula_gateway_client_manager:stop(Pid),
    timer:sleep(50),
...
client_connected_monitors_process_test_() ->
    {setup,
     ...
     fun(Pid) ->
        ...
        {ok, _Info} = macula_gateway_client_manager:get_client_info(Pid, ClientPid),
        ...
        [?_assertEqual(not_found, macula_gateway_client_manager:get_client_info(Pid, ClientPid))]
     end}.
...
client_connected_duplicate_updates_test_() ->
    {setup,
     ...
     fun(Pid) ->
        ...
        ok = macula_gateway_client_manager:client_connected(Pid, ClientPid, ClientInfo1),
        ok = macula_gateway_client_manager:client_connected(Pid, ClientPid, ClientInfo2),
...
get_client_info_not_found_test_() ->
    {setup,
     ...
     fun(Pid) ->
        ...
        [?_assertEqual(not_found, macula_gateway_client_manager:get_client_info(Pid, UnknownPid))]
     end}.
...
get_all_clients_empty_test_() ->
    {setup,
     ...
     fun cleanup/1,
     fun(Pid) ->
        {ok, Clients} = macula_gateway_client_manager:get_all_clients(Pid),
...
get_all_clients_multiple_test_() ->
    {setup,
     ...
     fun(Pid) ->
        ...
        macula_gateway_client_manager:client_connected(Pid, Client1,
            #{realm => <<"test">>, node_id => <<"node1">>}),
        macula_gateway_client_manager:client_connected(Pid, Client2,
...
is_client_alive_true_test_() ->
    {setup,
     ...
     fun(Pid) ->
        ...
        [?_assertEqual(true, macula_gateway_client_manager:is_client_alive(Pid, ClientPid))]
     end}.
...
is_client_alive_false_test_() ->
    {setup,
     ...
     fun(Pid) ->
        ...
        [?_assertEqual(false, macula_gateway_client_manager:is_client_alive(Pid, ClientPid))]
     end}.
...
store_client_stream_test_() ->
    {setup,
     ...
     fun(Pid) ->
        ...
        ok = macula_gateway_client_manager:store_client_stream(Pid, NodeId, Stream),

        {ok, StoredStream} = macula_gateway_client_manager:get_client_stream(Pid, NodeId),
...
get_client_stream_not_found_test_() ->
    {setup,
     ...
     fun cleanup/1,
     fun(Pid) ->
        [?_assertEqual(not_found, macula_gateway_client_manager:get_client_stream(Pid, <<"unknown">>))]
     end}.
...
store_client_stream_update_test_() ->
    {setup,
     ...
     fun(Pid) ->
        ...
        ok = macula_gateway_client_manager:store_client_stream(Pid, NodeId, Stream1),
        ok = macula_gateway_client_manager:store_client_stream(Pid, NodeId, Stream2),
...
concurrent_connections_test_() ->
    {setup,
     ...
     fun(Pid) ->
        ...
        [macula_gateway_client_manager:client_connected(Pid, C,
            #{realm => <<"test">>, node_id => integer_to_binary(I)})
         || {C, I} <- lists:zip(Clients, lists:seq(1, 10))],

        {ok, AllClients} = macula_gateway_client_manager:get_all_clients(Pid),
...
max_clients_limit_enforced_test_() ->
    {setup,
     ...
     fun(Pid) ->
        ...
        ok = macula_gateway_client_manager:client_connected(Pid, Client1,
            #{realm => <<"test">>, node_id => <<"node1">>}),
        ok = macula_gateway_client_manager:client_connected(Pid, Client2,
            #{realm => <<"test">>, node_id => <<"node2">>}),
        ok = macula_gateway_client_manager:client_connected(Pid, Client3,
        ...
        Result = macula_gateway_client_manager:client_connected(Pid, Client4,
            #{realm => <<"test">>, node_id => <<"node4">>}),

        %% Pool should still be 3 (4th client rejected)
        {ok, Clients2} = macula_gateway_client_manager:get_all_clients(Pid),
...
update_client_when_pool_full_test_() ->
    {setup,
     ...
     fun(Pid) ->
        ...
        UpdateResult = macula_gateway_client_manager:client_connected(Pid, Client1,
            #{realm => <<"updated">>, node_id => <<"node1_updated">>}),

        %% Verify update worked
        {ok, UpdatedInfo} = macula_gateway_client_manager:get_client_info(Pid, Client1),
