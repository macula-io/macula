...
| `macula-10.21.0-builders/` | macula-station | The EVENT, GOODBYE and STREAM_ERROR frames the captures never saw, built with the released macula 10.21.0 that station 311c0bf runs, the way the station's send paths build and sign them: EVENT through its pubsub server (a relayed PUBLISH with and without `publisher_sig`, and its own publish), GOODBYE as the peering connection sends it on close for each reason the station closes with, STREAM_ERROR for each refusal of a relayed stream. Each decoded back with that macula before it was written. `TYPE.SHAPE.bin`. 2026-09-14. |
| `macula-10.24.0-builders/` | macula-station | The same, with the released macula 10.24.0 that station 7d00b13 runs, plus the STREAM_ERROR for a failed forward that 7d00b13 adds. 2026-09-14. |
