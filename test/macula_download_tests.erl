...
single_block_get_reports_unchunked() ->
    ...
    {ok, _Pid} = macula_download:start_link(?MODULE, pool, <<0:256>>, ?SINGLE_MCID, self()),
...
manifest_get_reports_chunked() ->
    process_flag(trap_exit, true),
    meck:expect(macula, get_content, fun(_Pool, _Mcid) -> {ok, <<"bytes">>} end),
    {ok, _Pid} = macula_download:start_link(?MODULE, pool, <<0:256>>, ?MANIFEST_MCID, self()),
    ?assertEqual({downloaded, {ok, <<"bytes">>}}, wait_msg()),
...
cancel_before_get_resolves_announces_cancelled() ->
    ...
    {ok, Pid} = macula_download:start_link(?MODULE, pool, <<0:256>>, ?SINGLE_MCID, self()),
