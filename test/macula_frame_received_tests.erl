...
samples() ->
    ...
    [{connect, macula_frame:connect(#{node_id => key(), station_id => key(), realms => [key()],
     ...
     {find_value, macula_frame:find_value(#{key => key(), origin => key()}), [key, origin], ['after']},
     {value, macula_frame:value(#{key => key(), records => [Record]}), [key, records], [next]},
     {store, macula_frame:store(#{record => Record}), [record], []},
     {store_ack, macula_frame:store_ack(#{key => key(), signer => key(), record_version => id(),
                                          stored => true}), [key, signer, record_version, stored], []},
