...
test_publish_disconnected() ->
    ...
    Result = (catch macula:publish(Client, <<"test.topic">>, #{})),
...
test_publish_qos() ->
    ...
    Client = spawn(fun() -> ok end),
    timer:sleep(10), %% Ensure dead
    Result = (catch macula:publish(Client, Topic, Data, Opts)),
...
test_subscribe_callback() ->
    ...
    Client = spawn(fun() -> ok end),
    timer:sleep(10), %% Ensure dead
    Result = (catch macula:subscribe(Client, <<"test.topic">>, Callback)),
...
test_unsubscribe_invalid() ->
    ...
    Client = self(),
    Result = (catch macula:unsubscribe(Client, InvalidRef)),
...
test_multiple_subscriptions() ->
    ...
    Client = spawn(fun() -> ok end),
    timer:sleep(10), %% Ensure dead
    Result1 = (catch macula:subscribe(Client, <<"test.topic">>, Callback1)),
    Result2 = (catch macula:subscribe(Client, <<"test.topic">>, Callback2)),
...
test_topic_design() ->
    ...
    lists:foreach(fun(Topic) ->
        Result = (catch macula:publish(Client, Topic, #{})),
        ?assertMatch({'EXIT', {noproc, _}}, Result)
...
test_json_encoding() ->
    ...
    ?assertMatch({'EXIT', {noproc, _}},
                 (catch macula:publish(Client, <<"test">>, Data1))),
    ?assertMatch({'EXIT', {noproc, _}},
                 (catch macula:publish(Client, <<"test">>, Data2))),
    ?assertMatch({'EXIT', {noproc, _}},
                 (catch macula:publish(Client, <<"test">>, Data3))).
