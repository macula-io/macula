...
a_chunk_size_of_zero_is_refused_everywhere_test_() ->
    {timeout, 5,
     fun() ->
         ...
         ?assertEqual({error, invalid_manifest}, macula_manifest:verify(Zero, Data)),
         ?assertError(function_clause, macula_manifest:create(Data, #{chunk_size => 0}))
     end}.
...
a_chunk_count_other_than_the_chunks_listed_is_refused_test_() ->
    {timeout, 5,
     fun() ->
         ...
         [begin
              Changed = self_consistent(M#{chunk_count := N}),
...
a_size_the_chunks_do_not_cover_is_refused_test() ->
    ...
    [?assertEqual({error, invalid_manifest}, macula_manifest:from_wire(self_consistent(M#{size := S})))
...
%% not an integer. Empty content with no chunks is whole.
...
%% `Manifest' with its mcid recomputed from its canonical fields, the way a
%% peer can make a manifest up so that it matches the MCID it names.
self_consistent(#{name := Name, size := Size, chunk_size := ChunkSize,
                  chunk_count := ChunkCount, root_hash := RootHash} = Manifest) ->
    ...
    Manifest#{mcid := <<1, 16#56, Hash/binary>>}.
